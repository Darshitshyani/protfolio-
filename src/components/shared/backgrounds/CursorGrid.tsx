...
// Styles live in src/styles/globals.css (.cursor-grid / .cursor-grid__canvas):
...
const CursorGrid = ({
  cellSize = 70,
  color = '#1E90FF', // site accent; upstream ships a fuchsia demo colour
  radius = 140,
  falloff = 'smooth',
  holdTime = 400,
  fadeDuration = 800,
  ...
  useEffect(() => {
    ...
    // ── THE CANVAS BOX IS CACHED, NOT READ PER POINTER FRAME ──────────────
    // getBoundingClientRect() forces a style recalc + layout flush, and this
    // handler is bound to `window` — it fires for every pointer frame anywhere
    // on the page, alongside the other components that want a rect on the same
    // frame. On a page where dozens of elements are dirtying inline transforms,
    // each of those flushes is a full recalc. The box only moves when the page
    // scrolls or the element resizes, so it is marked dirty there and re-read
    // at most once per scroll rather than once per pointer move. (This renders
    // `fixed inset-0` on the home page, where it never moves at all — but the
    // component does not get to assume that.)
    let rectLeft = 0;
    let rectTop = 0;
    let rectDirty = true;
    ...
    // Reduced motion: paint the static grid once and stop. No rAF, no pointer
    // tracking — a cursor-reactive field is exactly the kind of motion that
    // preference asks us to suppress. Upstream has no such guard.
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
      rebuild();
      const roStatic = new ResizeObserver(() => rebuild());
    ...
    // window, not container: this renders as a pointer-events:none backdrop,
    // and such an element never receives pointer events itself. toLocal()
    // already maps clientX/Y through getBoundingClientRect, so coordinates are
    // correct either way.
    window.addEventListener('pointermove', onPointerMove);
    window.addEventListener('pointerdown', onPointerDown);
    // capture: true so a scroll in any scrolling ancestor invalidates too.
